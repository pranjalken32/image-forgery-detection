    ...
    for epoch in range(n_epochs):
        ...
        for i, (inputs, labels) in enumerate(train_loader):
            ...
            if (i + 1) % (print_every + 1) == 0:
                ...
                c += 1
        total_predicted_tensor = torch.tensor(total_predicted)
        total_predicted_np = total_predicted_tensor.cpu().numpy()
        total_labels_tensor = torch.tensor(total_labels)
        total_labels_np = total_labels_tensor.cpu().numpy()
        epoch_predictions = (total_predicted_np == total_labels_np).sum().item()
